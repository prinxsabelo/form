...
const Drawer = props => {
    ...
    const setQType = (typeId) => {
        const questionId = qn.questionId;
        developQuestion({ formId, typeId, qTypeAction, questionId });
        setDrawerIsOpen(false);

    }

    const { useQType, qn, questionTypes, developQuestion,
        setDrawerIsOpen, qTypeAction } = useContext(QuestionContext);


    //Initializing typeId to -1 .. To check and update if typeId is the current state for useQType.typeId 
    let typeId = -1;
    if (useQType && qTypeAction === "edit") {
        typeId = useQType.typeId;
    } else {
        typeId = -1;
    }
                ...
                <aside className={qTypeAction === "new" ? `drawer` : `edit-drawer`} onClick={props.onClick}>
                    ...
                    <div className="">
                        ...
                        {questionTypes.map((qt, index) =>
                            <div key={index} onClick={() => setQType(qt.typeId)}
                                className={`${typeId === qt.typeId ? 'bg-gray-200' : ''}
